...
const SellerKitchens = () => {
  ...
  const loadKitchens = async () => {
    try {
      const { data } = await apiClient.get("/admin/kitchens");
      setKitchens(data);
    } catch (err) {
    ...
    try {
      if (editingId) {
        const { data } = await apiClient.put(
          `/admin/kitchens/${editingId}`,
          formData,
        );
        setKitchens((prev) => prev.map((k) => (k.id === editingId ? data : k)));
        toast.success("Kitchen updated");
      } else {
        const { data } = await apiClient.post("/admin/kitchens", formData);
        setKitchens((prev) => [...prev, data]);
  ...
  const handleDelete = async (id) => {
    if (!window.confirm("Permanently delete this kitchen?")) return;
    try {
      await apiClient.delete(`/admin/kitchens/${id}`);
          ...
          <div className="bg-white rounded-xl border border-gray-100 p-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-semibold text-neutral-800">
                {editingId ? "Edit Kitchen" : "New Kitchen"}
              </h2>
              <button
                onClick={resetForm}
        ...
        <div className="bg-white rounded-xl border border-gray-100 overflow-hidden overflow-x-auto">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-16 gap-3">
              ...
              <p className="text-muted-foreground text-sm">No kitchens yet</p>
            </div>
          ) : (
