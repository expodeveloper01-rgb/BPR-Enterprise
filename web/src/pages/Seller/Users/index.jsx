...
const SellerUsers = () => {
  ...
  const handleRoleToggle = async (target) => {
    ...
    if (!window.confirm(`Are you sure you want to ${label} ${target.name}?`))
      return;
    try {
      const { data } = await apiClient.patch(`/admin/users/${target.id}/role`, {
        role: newRole,
      });
      setUsers((prev) => prev.map((u) => (u.id === data.id ? data : u)));
      toast.success(
        newRole === "admin"
          ? `${data.name} is now an admin`
          : `${data.name} is now a regular user`,
      );
    } catch (err) {
  ...
  const handleDelete = async (target) => {
    if (
      !window.confirm(
        `Permanently delete account for ${target.name} (${target.email})?`,
      )
    )
      return;
    try {
                ...
                {users.map((u) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-4 py-3">
                        <span
                          className={`text-xs font-semibold px-2.5 py-1 rounded-full ${isAdmin ? "bg-amber-50 text-amber-700" : "bg-gray-100 text-gray-600"}`}
                        >
                          {isAdmin ? "Admin" : "User"}
                        </span>
                      </td>
