...
const SellerSizes = () => {
  ...
  const loadSizes = async () => {
    try {
      const { data } = await apiClient.get("/admin/sizes");
      setSizes(data);
    } catch (err) {
    ...
    try {
      if (editingId) {
        const { data } = await apiClient.put(
          `/admin/sizes/${editingId}`,
          formData,
        );
        setSizes((prev) => prev.map((s) => (s.id === editingId ? data : s)));
        toast.success("Size updated");
      } else {
        const { data } = await apiClient.post("/admin/sizes", formData);
  ...
  const handleDelete = async (id) => {
    if (!window.confirm("Permanently delete this size?")) return;
    try {
    ...
    <SellerLayout>
      <div className="space-y-5">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-neutral-800">Sizes</h1>
